...
<c:forEach items="${smas}" var="member" varStatus="status">
		<tr style="background-color:${member.role.color}">
			...
			<c:choose>			<c:when test="${editable==true}">
				<td align="center">
					<a href="../acc/processAccount?method=update&code=${member.code}" >
					<link rel="xyz" type="text/css" href="color.css">
			...
			<c:when test="${subMember != null}">
				...
				<c:set var="sma" value="${subMember}" scope="request"/>
				<jsp:include page="smaDisplay.jsp"/>
			</c:when>
			</c:choose>
			<c:set var="editable" value="${initEditable}" scope="request"/>
			<c:set var="tab" value="${initSmaTab}" scope="request"/>
		</tr>
</c:forEach>
